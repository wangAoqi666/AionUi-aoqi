...
const AgentPillBar: React.FC<AgentPillBarProps> = ({
  ...
  const visibleAgents = availableAgents
    .filter((agent) => agent.backend !== 'custom' || agent.isExtension || (agent.customAgentId && !agent.isPreset))
    .filter((agent) => !showOnlySelected || getAgentKey(agent) === selectedAgentKey);
    ...
    <div className='w-full flex justify-center'>
      ...
      >
        ...
        {showDiscoverMoreButton && (
          <>
            {!isMobile && visibleAgents.length > 0 && (
