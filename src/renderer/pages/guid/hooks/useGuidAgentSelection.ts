...
export const useGuidAgentSelection = ({
  ...
  useEffect(() => {
    ...
    //
    // Align with the rest of this hook: Droid is the product's primary agent,
    // so prefer Droid, then the first CLI (non-preset) agent, with a final
    // fallback to the literal 'droid' key.
    if (resetAssistant && !resetHandledRef.current) {
      ...
      const fallbackKey = droidAgent ? getAgentKey(droidAgent) : firstCliAgent ? getAgentKey(firstCliAgent) : 'droid';
