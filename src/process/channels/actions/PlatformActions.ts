...
export const handlePairingShow: ActionHandler = async (context) => {
  ...
  if (await pairingService.isUserAuthorized(context.userId, platform, context.pluginId)) {
  ...
  try {
    ...
    return createSuccessResponse({
      ...
      text: [
        '🔗 <b>Device Pairing</b>',
        '',
        'Please approve this pairing request in the 智能体工厂 app:',
        '',
        `<code>${code}</code>`,
        '',
  ...
  try {
    ...
    return createSuccessResponse({
      ...
      text: [
        ...
        `⏱ Valid for: ${expiresInMinutes} minutes`,
        '',
        'Please approve this pairing request in 智能体工厂 settings.',
      ].join('\n'),
      parseMode: 'HTML',
      replyMarkup: getPairingCodeMarkup(platform, code),
    });
...
export const handlePairingCheck: ActionHandler = async (context) => {
  ...
  const pendingRequest = await pairingService.getPendingRequestForUser(context.userId, platform, context.pluginId);
  ...
  if (pendingRequest) {
    ...
    return createSuccessResponse({
      ...
      text: [
        ...
        '',
        'Please approve the pairing request in 智能体工厂 settings.',
      ].join('\n'),
      parseMode: 'HTML',
...
export const handlePairingHelp: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    ...
    text: [
      ...
      '',
      '<b>What is pairing?</b>',
      `Pairing links your ${platformName} account with the local 智能体工厂 application.`,
