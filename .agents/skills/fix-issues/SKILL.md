...
Then use `Read` to view the downloaded image. Codex can understand screenshots and extract:
...
"Internal error" on first message with Sonnet + Codex backend → searchable error path.
...
Do NOT include changes to `.Codex/skills/` in bug-fix branches. Skill updates should go through
