...
vi.mock('@/common', () => ({
  ipcBridge: {
    fs: {
      ...
      getFactoryGlobalPaths: { provider: vi.fn() },
      listFactoryRuleFiles: { provider: vi.fn() },
      ...
      readProjectFile: { provider: vi.fn() },
      writeProjectFile: { provider: vi.fn() },
      ensureProjectFiles: { provider: vi.fn() },
      readMcpJsonFile: { provider: vi.fn() },
      writeMcpJsonFile: { provider: vi.fn() },
    },
